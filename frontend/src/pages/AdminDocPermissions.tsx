  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((p) => !p)}
        className="flex items-center gap-2 text-xs font-semibold text-white uppercase tracking-wider mb-1.5 hover:text-white transition-colors"
      >
        {open ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
        {label}
        {selected.length > 0 && (
          <span className="ml-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-[rgba(59,130,246,0.2)] text-[#3B82F6]">
            {selected.length}
          </span>
        )}
      </button>
        ...
        <div className="grid grid-cols-2 gap-1 pl-4 max-h-40 overflow-y-auto">
          {options.map((opt) => (
            <label key={opt} className="flex items-center gap-2 text-xs cursor-pointer py-0.5 group">
              ...
              />
              <span className="text-white group-hover:text-white transition-colors truncate">{opt}</span>
            </label>
          ))}
        </div>
      )}
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="relative z-20 mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: "#FFFFFF", textShadow: "0 1px 10px rgba(0,0,0,0.28)" }}>
          <Shield className="w-6 h-6 text-[#3B82F6]" />
          Permisos Documentales
        </h1>
        <p className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.82)", textShadow: "0 1px 8px rgba(0,0,0,0.24)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 min-h-[calc(100vh-220px)]">
        ...
        <div className="premium-card flex flex-col overflow-hidden">
          <div className="p-3 border-b border-[rgba(62,74,95,0.4)]">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center h-24 text-white text-sm">Cargando...</div>
            ) : filteredUsers.length === 0 ? (
              <div className="flex items-center justify-center h-24 text-white text-sm">Sin resultados</div>
            ) : (
              filteredUsers.map((u) => (
                <button
                  key={u.id}
                  onClick={() => selectUser(u)}
                  className={`w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors hover:bg-white/[0.04] border-b border-[rgba(62,74,95,0.2)] last:border-0 ${
                    selected?.id === u.id ? "bg-[rgba(59,130,246,0.1)] border-l-2 border-l-primary" : "border-l-2 border-l-transparent"
                  }`}
                >
                  <div className="w-7 h-7 rounded-full bg-[rgba(59,130,246,0.15)] flex items-center justify-center flex-shrink-0">
                    <User className="w-3.5 h-3.5 text-[#3B82F6]" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold text-[#F3F7FC] truncate">
                      {u.fullName || u.email}
                    </p>
                    {u.fullName && (
                      <p className="text-[10px] text-white truncate">{u.email}</p>
                    )}
                  </div>
          ...
          {!selected || !access ? (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 p-8 text-center">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
                ...
                <Shield className="w-7 h-7 text-[#3B82F6] opacity-60" />
              </div>
              <div
                className="relative z-20"
                style={{ color: "#FFFFFF", textShadow: "0 1px 8px rgba(0,0,0,0.24)" }}
              >
                <p className="text-sm font-medium" style={{ color: "#FFFFFF", textShadow: "0 1px 8px rgba(0,0,0,0.24)" }}>Configura permisos documentales</p>
                <p className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.82)", textShadow: "0 1px 8px rgba(0,0,0,0.24)" }}>Selecciona un usuario de la lista para gestionar su acceso al RAG</p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col h-full">
              {/* Header */}
              <div className="p-4 border-b border-[rgba(62,74,95,0.4)] flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-[rgba(59,130,246,0.15)] flex items-center justify-center">
                  <User className="w-4 h-4 text-[#3B82F6]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#F3F7FC]">{selected.fullName || selected.email}</p>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {/* Allow all toggle */}
                <div className="flex items-center justify-between p-3 rounded-xl border border-[rgba(62,74,95,0.4)] bg-[#171C25]/30">
                  <div>
                    <p className="text-sm font-semibold text-[#F3F7FC]">Acceso completo</p>
                ...
                {!access.allowAll && (
                  <div className="space-y-4 p-3 rounded-xl border border-[rgba(62,74,95,0.4)] bg-[#171C25]/20">
                    <p className="text-xs font-bold text-[#F3F7FC] uppercase tracking-wider">
                      Filtros activos
                    </p>
                    ...
                    <label className="flex items-center gap-2.5 text-sm cursor-pointer">
                      ...
                      />
                      <span className="text-white text-xs">Solo documentos apostillados</span>
                    </label>
                    ...
                    <div className="pt-2 flex flex-wrap gap-1.5">
                      ...
                      {!filters.countries?.length && !filters.docTypes?.length && !filters.clients?.length && !filters.apostilledOnly && (
                        <p className="text-xs text-white italic">Sin filtros — el usuario no podrá ver ningún documento.</p>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Save button */}
              <div className="p-4 border-t border-[rgba(62,74,95,0.4)]">
